...
#### 2. [程式碼解釋實作](./02-code-explanation/) (15分鐘)  
- **@workspace 核心功能**：理解專案結構和程式碼關係
- **實作重點**：分析購物車系統，學習提問技巧
- **Agent 使用**：❌ 不使用（熟悉 @workspace）
...
#### 4. [偵錯輔助](./04-debugging-assistant/) (15分鐘)
- **Agent 錯誤診斷**：讓 Agent 主動發現和修復問題
- **實作重點**：展示 Agent 在複雜偵錯中的優勢
- **Agent 使用**：🟡 30%（體驗 Agent 智能診斷）

#### 🔥 **專業階段** - Agent 成為主角

#### 5. [單元測試](./05-unit-testing/) (15分鐘)
- **Agent 測試策略**：自主設計測試架構和用例
- **實作重點**：Agent 完整測試規劃 vs 手動寫測試
- **Agent 使用**：🟠 50%（Agent 開始主導工作流程）

#### 6. [程式碼重構](./06-code-refactoring/) (15分鐘)
- **Agent 重構規劃**：分析遺留代碼並制定重構策略
- **實作重點**：Agent 自主完成大規模重構任務
- **Agent 使用**：🟠 60%（展示 Agent 在複雜任務中的能力）

#### 🎯 **專家階段** - Agent 主導開發

#### 7. [文件生成](./07-documentation/) (10分鐘)
- **Agent 文件自動化**：從程式碼自動生成完整文件體系
- **實作重點**：Agent 自主分析並生成多種類型文件
- **Agent 使用**：🔴 80%（幾乎完全依靠 Agent）

#### 8. [綜合專案開發](./08-comprehensive-project/) (25分鐘)
- **⭐ Agent Mode 完全主導**：端到端自主開發
- **實作重點**：Agent 獨立完成任務管理系統的規劃、開發、測試、部署
- **Agent 使用**：🔴 90%（展示 Agent 的完整潛力）
